...
| 2 | **Voyage Map landing page** — the map *is* the homepage; ship docked at the live current-season port, cast on deck. **Folds in VOY-07: the 12 scene dioramas ARE the ports** — docking at a port opens that scene. | ▶ in progress |
| 3 | **Full-cast hero wall** — all 131 in one animated hall (B-02) | ⏳ next |
| 4 | **Clickable voyage islands** — click an island → its dungeon/raid data + completion state (X-05). Folds in the backend's island-completion feed. | ⏳ |
| 5 | **Unlock-the-Cast meter (ENG-10)** — generated-vs-remaining progress; un-generated members as wanted-poster silhouettes. Makes the rollout itself content. Sits well beside the hero wall / roster. | ⏳ |
