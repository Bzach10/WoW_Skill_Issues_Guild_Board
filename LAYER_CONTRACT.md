...
```
background  <  cloak  <  body  <  legs  <  chest  <  arms  <  head/face  <  weapons
   (scene)      10       20      30       40        50       60 / 61       70 / 71
```
...
| `head`, `face` | `head` | counter-bob (±0.9°) |
...
* non-numeric `anchor`/`pivot`/`z`, `layers` not a list, `canvas` garbage,
  the whole manifest not an object → contract defaults
